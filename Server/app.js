...
async function subscribeAndHandle() {
  try {
    ...
    // Connect and get MQTT message
    const message = await connectAndSubscribe();
    console.log("Connected and received message:", message);

    // Parse MQTT message
    let waterUsage;
    try {
      waterUsage = JSON.parse(message).totalLitres_water;
          //  waterUsage.pH = pH;
          //  waterUsage.turbidity = turbidity;
          //  waterUsage.chlorine = chlorine;
    } catch (parseError) {
    ...
    const deviceId = "develop.contact@gmail.com";
    // const entry = await deviceModel.findOne({ deviceId:deviceId });
    const entry = await deviceModel.findById('67cdacc1e8169ce387beed27')
